...
def crnn_forward(sent_pos, filter_size, num_filters1, num_filters2, layer_size, attention_size, keep_prob_rnn):
	...
	inputs = tf.unstack(conv, num=max_len, axis=1)

	with tf.variable_scope('rnn-%s' % filter_size):
	...
	with tf.variable_scope('attention-%s' % filter_size):
		w_omega = tf.Variable(tf.random_normal([2 * num_filters2, attention_size], stddev=0.1))
		b_omega = tf.Variable(tf.random_normal([attention_size], stddev=0.1))
		u_list = []
		for t in range(max_len):
			u_t = tf.tanh(tf.matmul(outputs[t], w_omega) + b_omega)
			u_list.append(u_t)
		attn_z = []
		u_omega = tf.Variable(tf.random_normal([attention_size, 1], stddev=0.1))
		for t in range(max_len):
			z_t = tf.matmul(u_list[t], u_omega)
			attn_z.append(z_t)
		attn_zconcat = tf.concat(attn_z, axis=1)
		alpha = tf.nn.softmax(attn_zconcat)
		alpha_trans = tf.reshape(tf.transpose(alpha, [1, 0]), [max_len, -1, 1])
		feature = tf.reduce_sum(outputs * alpha_trans, 0)
...
class CRNNModel(BaseModel):
	...
	def __init__(self, word_embed, data, word_dim,
		...
		for filter_size in filter_size_list:
			output = crnn_forward(sent_pos, filter_size,
								  num_filters1, num_filters2, layer_size, attention_size, keep_prob_rnn)
			outputs.append(output)
		feature = tf.reshape(tf.concat(outputs, 1), [-1, len(filter_size_list) * 2 * num_filters2])
