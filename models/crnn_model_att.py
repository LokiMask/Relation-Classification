...
def crnn_forward(name, sent_pos, filter_size_list, num_filters1, num_filters2, attention_size, layer_size, keep_prob_rnn):
	...
	conv_outputs = tf.unstack(convs, num=max_len, axis=1)
	inputs = []
	with tf.variable_scope('attention'):
		w_omega = tf.Variable(tf.random_normal([num_filters1, attention_size], stddev=0.1))
		b_omega = tf.Variable(tf.random_normal([attention_size], stddev=0.1))
		u_omega = tf.Variable(tf.random_normal([attention_size, 1], stddev=0.1))
		# import pdb
		# pdb.set_trace()
		for t in range(max_len):
			att_inputs = tf.unstack(conv_outputs[t], axis=1)
			u_list = []
			for i in range(len(att_inputs)):
				u_t = tf.tanh(tf.matmul(att_inputs[i], w_omega) + b_omega)
				u_list.append(u_t)
			attn_z = []
			for i in range(len(att_inputs)):
				z_t = tf.matmul(u_list[i], u_omega)
				attn_z.append(z_t)
			attn_zconcat = tf.concat(attn_z, axis=1)
			alpha = tf.nn.softmax(attn_zconcat)
			alpha_trans = tf.reshape(tf.transpose(alpha, [1, 0]), [len(att_inputs), -1, 1])
			output = tf.reduce_sum(att_inputs * alpha_trans, 0)
...
class CRNNATTModel(BaseModel):
	...
	def __init__(self, word_embed, data, word_dim,
		...
		feature = crnn_forward('crnn_att', sent_pos, filter_size_list, num_filters1, num_filters2,
							   attention_size, layer_size, keep_prob_rnn)
...
def build_train_valid_model(word_embed, train_data, test_data):
	with tf.name_scope("Train"):
		with tf.variable_scope('CRNNModel', reuse=None):
			m_train = CRNNATTModel(word_embed, train_data, FLAGS.word_dim,
								FLAGS.pos_num, FLAGS.pos_dim, FLAGS.num_relations,
								FLAGS.keep_prob, FLAGS.filter_size, FLAGS.num_filters1,
								FLAGS.num_filters2, FLAGS.attention_size, FLAGS.layer_size, FLAGS.keep_prob_rnn, FLAGS.lrn_rate, is_train=True)
	with tf.name_scope('Valid'):
		with tf.variable_scope('CRNNModel', reuse=True):
			m_valid = CRNNATTModel(word_embed, test_data, FLAGS.word_dim,
								FLAGS.pos_num, FLAGS.pos_dim, FLAGS.num_relations,
								1.0, FLAGS.filter_size, FLAGS.num_filters1,
								FLAGS.num_filters2, FLAGS.attention_size, FLAGS.layer_size, 1.0, FLAGS.lrn_rate, is_train=False)
